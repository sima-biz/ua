...
// node sitemap-generate-level1.js


import { row1level1initalDATA } from "./js/arrays-data/row1-level1-inital-DATA.js"; // Импорт данных
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" 
    ...
    const urls = data.map((item) => {
        return LANGUAGES.map((lang) => {
            ...
            const loc = escapeAmpersand(`${BASE_URL}/${item.hreflevel1abc}?a=${item.a}&b=${item.b || ""}&c=${item.c || ""}&d=&e=&f=&lang=${lang}`);

            // Генерация ссылок для всех альтернативных языков
            const alternateLinks = LANGUAGES.map((alternateLang) => {
                const alternateHref = escapeAmpersand(`${BASE_URL}/${item.hreflevel1abc}?a=${item.a}&b=${item.b || ""}&c=${item.c || ""}&d=&e=&f=&lang=${alternateLang}`);
...
const sitemapContent = generateSitemap(row1level1initalDATA);

// Запись карты сайта в файл
fs.writeFile("sitemap-level1.xml", sitemapContent, "utf8", (err) => {
    if (err) {
        console.error("Ошибка при записи файла:", err);
    } else {
        console.log("Карта сайта успешно создана в файле sitemap-level1.xml");
    }
});